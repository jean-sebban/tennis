...
don_with_stats2 <- don_with_stats %>% 
  ...
  mutate(nb_match_won_last_3 = nb_matchs(winner_or_loser,5)) %>%
  # mutate(avg_nb_ace = dplyr::lag(cummean(aces))) %>% 
  mutate(avg_nb_ace_last_3 = ace(x=aces, N=3)) %>%
...
don <- don_with_stats2 %>%
  ...
  select(contains(c("match_stats_url_suffix","numero_ordre","weight","height","age","classement","nb_match","avg_nb_ace")))
