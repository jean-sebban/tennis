...
## 2-7 Ajout de variables calculées
...
```{r}
tmp <- don %>% 
  mutate(classement_diff = classement_loser - classement_winner)
don <- tmp
rm(tmp)
```
...
4.  On stocke deux versions du fichier : avec valeurs manquantes (pour
    tester les imputations) et sans valeurs manquantes

```{r}
#1
don <- don %>% select(target,
                      contains(c("j1","j2")),
                      tourney_surface,
                      tourney_conditions,
                      classement_diff,
...
#4
don_sansNA <- na.omit(don_avecNA) # on enlève toutes les valeurs manquantes
...
Le fichier d'entraînement est constitué des matches entre 1991 et 2015.
Le fichier test est celui des matches les plus récents : 2016 et 2017.
...
#fichier de test
don_test <- don %>% filter(substr(match_stats_url_suffix,12,15) %in% c("2016","2017")) %>% 
  select(-match_stats_url_suffix)

#fichier d'entraînement
don_train <- don %>% filter(!substr(match_stats_url_suffix,12,15) %in% c("2016","2017")) %>% 
  select(-match_stats_url_suffix)
...
Test avec une validation croisée à 10 blocs sur les données d'entraînement. Les résultats des différents algorithmes seront stockés dans le dataframe RES.
...
Notre modèle doit être capable de prédire aussi bien les matches perdus que les matches gagnés. 
