...
-   4 - pourcentage de points gagnés au premier service sur les x matches précédents
-   5 - pourcentage de points gagnés au deuxième service sur les x matches précédents
...
Dans les tournois récents, il n'y a plus de tournois sur moquette, donc il faut enlever ces matches des modèles parce qu'ils ne pourront pas être utilisés dans les matches récents.
...
Le but de l'exploration de la base est de comprendre sa structure, notamment les liens entre les différentes variables.
...
res.pca <- PCA(dplyr::sample_n(data, 1000)[,-c(1,23)],quali.sup = 21,quanti.sup = 22)
...
plot(res.pca, choix="ind",select = "cos2 0.6",habillage = 21)
plot(res.pca, choix="var",habillage=21)
...
## 2 - Influence des variables avec l'arbre de regression
...
Le problème de cette méthode telle qu'elle est développée ci-après est qu'elle ne considère pas les données comme inscrites dans le temps. Les matches du passé seront utilisés pour prédire les matches à venir.
...
## 3 - Validation out of bag

Le fichier d'entraînement créé plus haut est divisé en un fichier d'entraînement et un fichier de validation.

```{r}
#fichier de validation
don_train_valid <- don  %>% filter(substr(match_stats_url_suffix,12,15) %in% c("2013","2014")) %>% 
  select(-match_stats_url_suffix)

#fichier d'apprentissage contenant tous les matches de 1991 à 2012 
#auquel on enlève les matches sur moquette qui ne sont pas dans les matches récents
don_train_app <- don %>% filter(!substr(match_stats_url_suffix,12,15) %in% c("2013","2014","2015","2016","2017")) %>% 
  select(-match_stats_url_suffix) %>% 
  filter(tourney_surface != "carpet")
...
L'algorithme retenu est donc le random forest. On le recalcule sur tout le fichier d'entraînement (en intégrant les matches des annéees 2013 à 2014). Et on le généralise sur le fichier qui n'a pas du tout été utilisé pour construire  le modèle : le fichier test (avec les matches des années 2015, 2016 et 2017).

Les matches sur moquette ne sont pas utilisés dans l'algorithme.


```{r}

#fichier d'apprentissage contenant tous les matches de 1991 à 2014 
#auquel on enlève les matches sur moquette qui ne sont pas dans les matches récents
don_train <- don_train  %>% 
  filter(tourney_surface != "carpet")
...
# #Fichier d'apprentissage pour gradient boosting
# don_train <- don_train %>% mutate(target = as.numeric(target),
#                                              target = case_when(
#                                                target == 1 ~ 0,
#                                                target == 2 ~ 1))  
#   
# 
# #fichier de validation pour gradient boosting
# don_test <- don_test %>% mutate(target = as.numeric(target),
#                                              target = case_when(
#                                                target == 1 ~ 0,
#                                                target == 2 ~ 1)) 
```

## Application du random forest sur le fichier d'entraînement

```{r}
RES <- data.frame(Y=don_test$target)
algo5 <- randomForest(target~.,data=don_train)
RES[,"foret"] <- predict(algo5,don_test,type="prob")[,2]
