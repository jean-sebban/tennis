...
author: "Jean Sebban, Fally Sow et Guillaume Allard"
format: revealjs
editor: visual
---
...
```{r, echo=TRUE}
algo9 <- gbm(target~.,data=dplyr::sample_n(don_train_app_gb,10000),distribution = "adaboost",n.trees = 3000,shrinkage = 0.01,cv.folds = 5)
...
```{r, echo=TRUE}
algo10 <- gbm(target~.,data=dplyr::sample_n(don_train_app_gb,10000),distribution = "bernoulli",n.trees = 3000,shrinkage = 0.05,cv.folds = 5)
...
On calcule les probabilités sur le fichier de validation (matches de 2013 et 2014).
...
On choisit le taux de bien classés comme critère de performance du modèle. 
